// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.lechnerc77.pulumi.btp.btp;

import com.lechnerc77.pulumi.btp.btp.GlobalaccountTrustConfigurationArgs;
import com.lechnerc77.pulumi.btp.btp.Utilities;
import com.lechnerc77.pulumi.btp.btp.inputs.GlobalaccountTrustConfigurationState;
import com.pulumi.core.Output;
import com.pulumi.core.annotations.Export;
import com.pulumi.core.annotations.ResourceType;
import com.pulumi.core.internal.Codegen;
import java.lang.Boolean;
import java.lang.String;
import javax.annotation.Nullable;

/**
 * Establishes trust from a global account to an Identity Authentication tenant.
 * 
 * __Further documentation:__
 * &lt;https://help.sap.com/docs/btp/sap-business-technology-platform/trust-and-federation-with-identity-providers&gt;
 * 
 * ## Example Usage
 * ```java
 * package generated_program;
 * 
 * import com.pulumi.Context;
 * import com.pulumi.Pulumi;
 * import com.pulumi.core.Output;
 * import com.pulumi.btp.GlobalaccountTrustConfiguration;
 * import com.pulumi.btp.GlobalaccountTrustConfigurationArgs;
 * import java.util.List;
 * import java.util.ArrayList;
 * import java.util.Map;
 * import java.io.File;
 * import java.nio.file.Files;
 * import java.nio.file.Paths;
 * 
 * public class App {
 *     public static void main(String[] args) {
 *         Pulumi.run(App::stack);
 *     }
 * 
 *     public static void stack(Context ctx) {
 *         var simple = new GlobalaccountTrustConfiguration(&#34;simple&#34;, GlobalaccountTrustConfigurationArgs.builder()        
 *             .identityProvider(&#34;terraformint.accounts400.ondemand.com&#34;)
 *             .build());
 * 
 *         var fullyCustomized = new GlobalaccountTrustConfiguration(&#34;fullyCustomized&#34;, GlobalaccountTrustConfigurationArgs.builder()        
 *             .description(&#34;my-description&#34;)
 *             .identityProvider(&#34;terraformint.accounts400.ondemand.com&#34;)
 *             .origin(&#34;my-own-origin-platform&#34;)
 *             .build());
 * 
 *     }
 * }
 * ```
 * 
 * ## Import
 * 
 * terraform
 * 
 * ```sh
 *  $ pulumi import btp:index/globalaccountTrustConfiguration:GlobalaccountTrustConfiguration terraform importbtp_globalaccount_trust_configuration.&lt;resource_name&gt; &lt;origin&gt;
 * ```
 * 
 * ```sh
 *  $ pulumi import btp:index/globalaccountTrustConfiguration:GlobalaccountTrustConfiguration trust sap.custom
 * ```
 * 
 */
@ResourceType(type="btp:index/globalaccountTrustConfiguration:GlobalaccountTrustConfiguration")
public class GlobalaccountTrustConfiguration extends com.pulumi.resources.CustomResource {
    /**
     * Description of the trust configuration.
     * 
     */
    @Export(name="description", refs={String.class}, tree="[0]")
    private Output<String> description;

    /**
     * @return Description of the trust configuration.
     * 
     */
    public Output<String> description() {
        return this.description;
    }
    /**
     * The tenant&#39;s domain which should be used for user logon.
     * 
     */
    @Export(name="domain", refs={String.class}, tree="[0]")
    private Output<String> domain;

    /**
     * @return The tenant&#39;s domain which should be used for user logon.
     * 
     */
    public Output<String> domain() {
        return this.domain;
    }
    /**
     * The name of the Identity Authentication tenant that you want to connect to the global account.
     * 
     */
    @Export(name="identityProvider", refs={String.class}, tree="[0]")
    private Output<String> identityProvider;

    /**
     * @return The name of the Identity Authentication tenant that you want to connect to the global account.
     * 
     */
    public Output<String> identityProvider() {
        return this.identityProvider;
    }
    /**
     * The display name of the trust configuration.
     * 
     */
    @Export(name="name", refs={String.class}, tree="[0]")
    private Output<String> name;

    /**
     * @return The display name of the trust configuration.
     * 
     */
    public Output<String> name() {
        return this.name;
    }
    /**
     * The origin of the identity provider.
     * 
     */
    @Export(name="origin", refs={String.class}, tree="[0]")
    private Output<String> origin;

    /**
     * @return The origin of the identity provider.
     * 
     */
    public Output<String> origin() {
        return this.origin;
    }
    /**
     * The protocol used to establish trust with the identity provider.
     * 
     */
    @Export(name="protocol", refs={String.class}, tree="[0]")
    private Output<String> protocol;

    /**
     * @return The protocol used to establish trust with the identity provider.
     * 
     */
    public Output<String> protocol() {
        return this.protocol;
    }
    /**
     * Shows whether the trust configuration can be modified.
     * 
     */
    @Export(name="readOnly", refs={Boolean.class}, tree="[0]")
    private Output<Boolean> readOnly;

    /**
     * @return Shows whether the trust configuration can be modified.
     * 
     */
    public Output<Boolean> readOnly() {
        return this.readOnly;
    }
    /**
     * Determines whether the identity provider is currently &#39;active&#39; or &#39;inactive&#39;.
     * 
     */
    @Export(name="status", refs={String.class}, tree="[0]")
    private Output<String> status;

    /**
     * @return Determines whether the identity provider is currently &#39;active&#39; or &#39;inactive&#39;.
     * 
     */
    public Output<String> status() {
        return this.status;
    }
    /**
     * The trust type.
     * 
     */
    @Export(name="type", refs={String.class}, tree="[0]")
    private Output<String> type;

    /**
     * @return The trust type.
     * 
     */
    public Output<String> type() {
        return this.type;
    }

    /**
     *
     * @param name The _unique_ name of the resulting resource.
     */
    public GlobalaccountTrustConfiguration(String name) {
        this(name, GlobalaccountTrustConfigurationArgs.Empty);
    }
    /**
     *
     * @param name The _unique_ name of the resulting resource.
     * @param args The arguments to use to populate this resource's properties.
     */
    public GlobalaccountTrustConfiguration(String name, GlobalaccountTrustConfigurationArgs args) {
        this(name, args, null);
    }
    /**
     *
     * @param name The _unique_ name of the resulting resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param options A bag of options that control this resource's behavior.
     */
    public GlobalaccountTrustConfiguration(String name, GlobalaccountTrustConfigurationArgs args, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        super("btp:index/globalaccountTrustConfiguration:GlobalaccountTrustConfiguration", name, args == null ? GlobalaccountTrustConfigurationArgs.Empty : args, makeResourceOptions(options, Codegen.empty()));
    }

    private GlobalaccountTrustConfiguration(String name, Output<String> id, @Nullable GlobalaccountTrustConfigurationState state, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        super("btp:index/globalaccountTrustConfiguration:GlobalaccountTrustConfiguration", name, state, makeResourceOptions(options, id));
    }

    private static com.pulumi.resources.CustomResourceOptions makeResourceOptions(@Nullable com.pulumi.resources.CustomResourceOptions options, @Nullable Output<String> id) {
        var defaultOptions = com.pulumi.resources.CustomResourceOptions.builder()
            .version(Utilities.getVersion())
            .build();
        return com.pulumi.resources.CustomResourceOptions.merge(defaultOptions, options, id);
    }

    /**
     * Get an existing Host resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state
     * @param options Optional settings to control the behavior of the CustomResource.
     */
    public static GlobalaccountTrustConfiguration get(String name, Output<String> id, @Nullable GlobalaccountTrustConfigurationState state, @Nullable com.pulumi.resources.CustomResourceOptions options) {
        return new GlobalaccountTrustConfiguration(name, id, state, options);
    }
}
